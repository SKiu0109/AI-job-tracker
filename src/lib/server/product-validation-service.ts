...
export interface ProductValidationService {
  ...
  listFeedback(limit?: number): Promise<AdminFeedbackListResponse>;
}
...
class InMemoryProductValidationService implements ProductValidationService {
  ...
  async listFeedback(limit = 100) {
    return buildFeedbackListResponse(
      [...this.feedback]
...
class SupabaseProductValidationService implements ProductValidationService {
  ...
  async listFeedback(limit = 100) {
    const response = await fetch(
      `${this.restUrl}/feedback?select=id,guest_id,role,goal,feedback,email,rating,language,path,created_at&order=created_at.desc&limit=${limit}`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Supabase feedback read failed: ${response.status} ${errorText}`
      );
    }

    const rows = (await response.json()) as FeedbackRow[];

    return buildFeedbackListResponse(rows.map(mapFeedbackRow));
  }
...
class FallbackProductValidationService implements ProductValidationService {
  ...
  async listFeedback(limit = 100) {
    try {
      return await this.primary.listFeedback(limit);
    } catch (error) {
      console.warn("[product-feedback-list-fallback]", error);
      return this.fallback.listFeedback(limit);
    }
  }
}
...
function mapFeedbackRow(row: FeedbackRow): AdminFeedbackItem {
  return {
...
function buildFeedbackListResponse(
  ...
  return {
    ...
    stats: {
      ...
      withEmail: feedback.filter((item) => Boolean(item.email)).length,
      zhCount: feedback.filter((item) => item.language === "zh").length
    }
  };
}
