...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {jobs.length === 0 ? (
        <section className="rounded-md border border-line bg-white px-6 py-12 text-center shadow-soft">
          <h2 className="text-xl font-semibold text-ink">{t.noAnalyticsYet}</h2>
          <p className="mx-auto mt-2 max-w-lg text-sm text-muted">{t.emptyBody}</p>
...
function MetricCard({
  ...
  return (
    <div className="rounded-md border border-line bg-white p-4 shadow-soft">
      ...
      <div className={compact ? "mt-2 text-2xl font-semibold" : "mt-3 text-3xl font-semibold"}>
        {value}
      </div>
    </div>
  );
    ...
    <div className="space-y-3">
      {jobs.map((job) => (
        <Link
          key={job.id}
          href={`/jobs/${job.id}`}
          className="block rounded-md border border-line bg-paper p-3 transition hover:border-accent hover:bg-white"
        >
...
function buildAnalytics(jobs: JobRecord[]) {
  ...
  return {
    ...
    averageMatchScore,
    highMatchJobs,
    jobsByRoleType: countValues(jobs.map((job) => job.job_type_en)),
    averageMatchByRole: averageScoresByGroup(jobs, (job) => job.job_type_en),
    ...
    regionBreakdown: countValues(jobs.map((job) => getRegionLabel(job.location))),
...
function getRegionLabel(location: string) {
